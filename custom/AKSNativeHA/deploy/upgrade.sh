...
kubectl delete -f mtlsqm.yaml -n $TARGET_NAMESPACE

( echo "cat <<EOF" ; cat mtlsqm.yaml_template ; echo EOF ) | sh > mtlsqm.yaml
kubectl apply -f mtlsqm.yaml -n $TARGET_NAMESPACE

helm upgrade nativeha ../../../charts/ibm-mq -f secureapp_nativeha.yaml -n $TARGET_NAMESPACE

manualAcceptance=true ../../genericresources/rollingupdate/demoRollingUpdate.sh nativeha-ibm-mq $TARGET_NAMESPACE
