...
export PORT="1414"
export IPADDRESS="$(kubectl get service nativeha-ibm-mq-loadbalancer -o jsonpath='{..ip}' -n $TARGET_NAMESPACE)"

( echo "cat <<EOF" ; cat ccdt_template.json ; echo EOF ) | sh > ccdt_generated.json

echo "Starting amqsphac" QMNHA
/opt/mqm/samp/bin/amqsphac APPQ QMNHA
